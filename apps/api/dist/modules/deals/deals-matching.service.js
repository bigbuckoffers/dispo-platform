...
var DealsMatchingService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.DealsMatchingService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../shared/prisma/prisma.service");
const openai_1 = require("openai");
let DealsMatchingService = DealsMatchingService_1 = class DealsMatchingService {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(DealsMatchingService_1.name);
        this.GATE_MIN_FINANCIAL_SCORE = 35;
        this.MAX_MATCHES = 355;
        this.openai = new openai_1.default({ apiKey: process.env.OPENAI_API_KEY });
    }
    async runMatchingForDeal(dealId) {
        ...
        if (!gateResult.passes) {
            await this.prisma.deal.update({ where: { id: dealId }, data: { matchedBuyerCount: 0, tier1MatchCount: 0, buyerDemandScore: 0, nextBestAction: gateResult.recommendation, buyerCoverageStatus: 'Not Ready' } });
        ...
        await this.prisma.deal.update({ where: { id: dealId }, data: { matchedBuyerCount: matches.length, tier1MatchCount: tier1Count, buyerDemandScore, sniperScore: this.calculateSniperScore(matches, deal), status: matches.length > 0 ? 'MATCHED' : deal.status, buyerCoverageStatus: this.getBuyerCoverageStatus(matches.length, tier1Count), nextBestAction: matches.length > 0 ? 'Send buyer blast' : 'Find buyers for this market' } });
    ...
    runFinancialGate(deal) {
        ...
        if (refValue && price) {
            ...
            if (ratio <= 0.60)
                ...
                gateReason = 'Price is ' + Math.round(ratio * 100) + '% of value — not enough discount';
        }
        else if (!refValue) {
            score += 15;
        ...
        if (dealType === 'SUBTO')
            score += 20;
        else if (['WHOLESALE', 'OWNER_FINANCE', 'NOVATION'].includes(dealType))
            score += 15;
        else if (dealType)
            score += 8;
        else
            dataGaps.push('Deal type not specified');
        if (deal.sourceType === 'OWN')
            score += 20;
        else if (deal.dealSource && deal.dealSource.reliabilityLabel === 'BLACKLIST') {
            score -= 50;
            gateReason = 'Source is blacklisted';
        }
        else
            score += 10;
        if (deal.address && deal.city && deal.state)
            ...
            dataGaps.push('Full address incomplete');
        if (deal.beds && deal.baths)
            score += 5;
        else
            dataGaps.push('Beds/baths missing');
        if (!deal.photos || !deal.photos.length)
            dataGaps.push('No photos — add before buyer blast');
        if (!deal.repairEstimate)
            dataGaps.push('Repair estimate missing');
        if (!deal.accessInfo)
            dataGaps.push('Access info missing');
        ...
        if (passes && !gateReason)
            gateReason = 'Financial score ' + financialScore + '/100 — qualifies for matching';
    ...
    preFilterBuyers(buyers, deal) {
        ...
        return buyers.filter(buyer => {
            ...
            if (bb && bb.states && bb.states.length > 0) {
                const stateMatch = bb.states.some((s) => s.toUpperCase() === dealState);
                if (!stateMatch)
                    return false;
            }
            else {
                ...
                if (!hasSignals || (!primaryMatch && !summaryMatch && !intelMatch))
                    return false;
            }
            if (bb && bb.maxPrice && price > 0 && price > bb.maxPrice * 1.5)
                return false;
            if (bb && bb.minPrice && price > 0 && price < bb.minPrice * 0.5)
                return false;
            return true;
        });
    }
    async aiScoreBuyer(buyer, deal) {
        const bb = buyer.buyBox;
        const buyBoxSummary = [
        ...
        let result = {};
        try {
            result = JSON.parse((response.choices[0]?.message?.content || '{}').replace(/```json|```/g, '').trim());
        }
        catch {
            result = { matchScore: 0, matchStrength: 'WEAK', matchReasoning: 'AI scoring failed', redFlags: [], outreachAngle: '', estimatedOfferRange: 'unknown' };
        }
    ...
    generateMatchSummary(deal, matches) {
        if (matches.length === 0)
            return 'No buyers matched for ' + deal.address + '. Consider expanding your buyer list for ' + deal.state + '.';
        const strong = matches.filter(m => m.matchStrength === 'STRONG');
        return ['Found ' + matches.length + ' matched buyers for ' + deal.address + ', ' + deal.city + ' ' + deal.state + '.', strong.length > 0 ? strong.length + ' strong match' + (strong.length > 1 ? 'es' : '') + ': ' + strong.slice(0, 3).map((m) => m.buyerName).join(', ') + '.' : null, matches[0] ? 'Top match: ' + matches[0].buyerName + ' (' + matches[0].matchScore + '/100).' : null].filter(Boolean).join(' ');
    }
    async persistResults(dealId, matches) {
    ...
    async getMatchesForDeal(dealId, limit = 50) {
    ...
    calculateSniperScore(matches, deal) {
        if (matches.length === 0)
            return 0;
        const top3 = matches.slice(0, 3);
        const avgTopScore = top3.reduce((s, m) => s + (m.matchScore || 0), 0) / top3.length;
        ...
        const avgBuyerQuality = top3.reduce((s, m) => s + (m.compositeScore || 50), 0) / top3.length;
    ...
    runFinancialGateOnly(deal) { return this.runFinancialGate(deal); }
    getBuyerCoverageStatus(matchCount, tier1Count) {
        if (matchCount >= 10 && tier1Count >= 2)
            return 'Strong Coverage';
        if (matchCount >= 5 || tier1Count >= 1)
...
exports.DealsMatchingService = DealsMatchingService;
exports.DealsMatchingService = DealsMatchingService = DealsMatchingService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], DealsMatchingService);
//# sourceMappingURL=deals-matching.service.js.map