...
import { Eye, Heart, Send, CheckCircle, Mail, MessageSquare, UserPlus, AlertTriangle, ShoppingCart } from 'lucide-react';
      ...
      {events.map((ev: any) => {
        ...
        return (
          <div key={ev.id} className="flex items-center gap-3">
            <div className={`flex-shrink-0 ${config.color}`}><config.icon size={14} /></div>
...
export function BuyerActivityChart() {
  ...
  if (!velocity.length) return <div className="h-48 flex items-center justify-center text-sm text-gray-600">Add deals to see activity</div>;
  return (
    <div className="h-48">
