...
const INTAKE_EVENT_LABELS: Record<string,string> = { INTAKE_LINK_CREATED:'Link created', INTAKE_LINK_SENT:'Link sent', INTAKE_LINK_OPENED:'Buyer opened form', INTAKE_FORM_STARTED:'Buyer started form', INTAKE_FORM_SUBMITTED:'Buyer submitted form', INTAKE_REMINDER_SENT:'Reminder sent', INTAKE_LINK_EXPIRED:'Link expired', INTAKE_MANUAL_REVIEW_NEEDED:'Manual review needed' };
const INTAKE_STEPS = [['INTAKE_LINK_CREATED','Created'],['INTAKE_LINK_SENT','Sent'],['INTAKE_LINK_OPENED','Opened'],['INTAKE_FORM_STARTED','Started'],['INTAKE_FORM_SUBMITTED','Submitted']];
const formatIntakeDate = (value?: string | Date | null) => value ? new Date(value).toLocaleString([], { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : '—';
const intakeStatusLabel = (status?: string) => status ? status.replace(/_/g,' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase()) : 'Not Sent';
...
export default function BuyerProfilePage({ params }: { params: { id: string } }) {
  ...
  const intakeEventsList = Array.isArray(intakeEvents) ? intakeEvents : [];
  const getIntakeEventDate = (type: string) => intakeEventsList.find((e:any)=>e.eventType===type)?.createdAt;
  ...
  const intakeDates = [['Created',getIntakeEventDate('INTAKE_LINK_CREATED')],['Sent',buyer.intakeSentAt||getIntakeEventDate('INTAKE_LINK_SENT')],['Opened',buyer.intakeOpenedAt||getIntakeEventDate('INTAKE_LINK_OPENED')],['Started',buyer.intakeStartedAt||getIntakeEventDate('INTAKE_FORM_STARTED')],['Submitted',buyer.intakeSubmittedAt||getIntakeEventDate('INTAKE_FORM_SUBMITTED')]];
  ...
  const checks=[{label:'Primary market',done:!!buyer.marketPrimary,p:3},{label:'Price range',done:!!(buyer.buyBox?.minPrice||buyer.buyBox?.maxPrice||buyer.buyBox?.anyPrice),p:5},{label:'Rehab tolerance',done:!!buyer.buyBox?.rehabTolerance,p:4},{label:'Strategy confirmed',done:!!(buyer.preferredStrategies?.length),p:3},{label:'Funding type',done:!!buyer.notes,p:4},{label:'Proof of funds',done:!!(buyer.proofOfFundsUrl||buyer.proofOfFundsWaived),p:5},{label:'Zip codes',done:!!(buyer.buyBox?.zipCodes?.length||buyer.buyBox?.anyZipOk),p:3},{label:'Intel notes',done:!!(buyer.buyerIntelNotes?.length>50),p:4},{label:'Phone verified',done:!!buyer.phone,p:2}];
