...
import {
  Shield, TrendingUp, Zap, Star,
  DollarSign, Phone, Mail, Building2, RefreshCw, Brain,
} from 'lucide-react';
...
export default function BuyerProfilePage({ params }: { params: { id: string } }) {
  ...
  if (isError || !buyer) return (
    <div className="p-6 text-red-400 text-sm">
      Could not load buyer.{' '}
      <a href="/dashboard/buyers" className="underline">Go back</a>
    </div>
  );
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            ...
            {(buyer.buyBox.minPrice || buyer.buyBox.maxPrice) && (
              <div>
                ...
                <p className="text-white mt-1">
                  {buyer.buyBox.minPrice ? formatCurrency(buyer.buyBox.minPrice) : '—'} – {buyer.buyBox.maxPrice ? formatCurrency(buyer.buyBox.maxPrice) : '—'}
                </p>
              </div>
            )}
            {buyer.buyBox.propertyTypes?.length > 0 && (
              <div>
                ...
                <p className="text-white mt-1">{buyer.buyBox.propertyTypes.join(', ')}</p>
              </div>
            )}
            {buyer.buyBox.rehabTolerance && (
              <div>
                <span className="text-gray-500">Rehab Tolerance</span>
                <p className="text-white mt-1">{buyer.buyBox.rehabTolerance}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <p className="text-white text-xl font-bold mt-1">{analytics.totalClosed ?? 0}</p>
            </div>
            <div>
              <span className="text-gray-500">Close Rate</span>
              <p className="text-white text-xl font-bold mt-1">{analytics.closeRate ?? 0}%</p>
            </div>
            <div>
              <span className="text-gray-500">Avg Fee</span>
              <p className="text-white text-xl font-bold mt-1">{analytics.avgFee ? formatCurrency(analytics.avgFee) : '—'}</p>
            </div>
            <div>
              <span className="text-gray-500">Ghost Rate</span>
              <p className="text-white text-xl font-bold mt-1">{analytics.ghostRate ?? 0}%</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
        <h2 className="text-white font-semibold mb-4 flex items-center gap-2">
          <Brain size={16} />Activity Timeline
        </h2>
