...
const INTAKE_STEPS = [['INTAKE_LINK_SENT','Sent'],['INTAKE_LINK_OPENED','Opened'],['INTAKE_FORM_STARTED','Started'],['INTAKE_FORM_SUBMITTED','Submitted']];
...
const intakeStatusLabel = (status?: string) => status === 'LINK_CREATED' ? 'Ready to Send' : status ? status.replace(/_/g,' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase()) : 'Not Sent';
...
export default function BuyerProfilePage({ params }: { params: { id: string } }) {
  ...
  const getIntakeReminderMessage = () => {
    ...
    const reminderNumber = getNextReminderNumber();
    ...
    if (reminderNumber === 2) {
      return `Hey, just following up — once your buy box is complete, we’ll know exactly what deals to send you instead of blasting you with stuff that doesn’t fit: ${link}`;
    }
  ...
  const sendIntakeReminder = async () => {
    ...
    try {
      await api.post(`/messages/conversations/${id}/send`, {
        message: getIntakeReminderMessage(),
        intakeTrackingType: 'reminder',
        reminderNumber: getNextReminderNumber(),
      });
      ...
      {showReminderConfirm&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm px-4">
          <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-blue-700/40 bg-gray-950 shadow-2xl">
            <div className="border-b border-gray-800 bg-gradient-to-r from-blue-950/80 to-purple-950/50 px-6 py-5">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-blue-500/30 bg-blue-600/20 text-blue-300">
                  💬
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-white">Send reminder SMS?</h3>
                  <p className="text-sm text-gray-400">This will text the buyer through DispoAI and log a reminder event.</p><p className="mt-1 text-xs text-blue-300">Reminder #{getNextReminderNumber()} template selected automatically.</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 px-6 py-5">
              ...
              <div className="rounded-xl border border-yellow-700/30 bg-yellow-900/10 p-3 text-xs text-yellow-200">
                This is a real SMS. Only send if you want to follow up with this buyer now.
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-800 px-6 py-4">
              ...
              <button
                onClick={sendIntakeReminder}
                disabled={sendingReminder || !buyer?.intakeToken}
                className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-50"
              >
                {sendingReminder ? 'Sending...' : `Send Reminder #${getNextReminderNumber()}`}
              </button>
            </div>
          </div>
        </div>
