...
function SubmissionReviewModal({ sub, onClose, onSave }: { sub: any, onClose: ()=>void, onSave: (data:any)=>void }) {
  const d = sub.submittedData || {};
  const b = sub.buyer || {};
  const bb = b.buyBox || {};

  // Initialize form with current buyer data
  ...
  const hasConflict = (k: string) => {
    const cur = String(form[k]||'').trim();
    const sub = String(submitted[k]||'').trim();
    return cur && sub && cur !== sub;
  };

  const isNew = (k: string) => {
    const cur = String(form[k]||'').trim();
    const sub = String(submitted[k]||'').trim();
    return !cur && sub;
  };

  // Field label renderer with conflict highlighting
  const Field = ({ label, fieldKey, type='text', options=null }: any) => {
    const conflict = hasConflict(fieldKey);
    const newVal = isNew(fieldKey);
    const subVal = submitted[fieldKey];
    return (
      <div className={`rounded-lg p-2 ${conflict ? 'bg-yellow-500/5 border border-yellow-500/20' : newVal ? 'bg-green-500/5 border border-green-500/20' : 'bg-transparent'}`}>
        <label className="text-gray-500 text-xs block mb-1">{label}
          {conflict && <span className="ml-2 text-yellow-400 text-xs">⚠ Conflict</span>}
          {newVal && <span className="ml-2 text-green-400 text-xs">✓ New</span>}
        </label>
        {conflict && (
          <div className="flex gap-2 mb-1.5 flex-wrap">
            <span className="text-xs text-yellow-300 bg-yellow-500/10 px-2 py-0.5 rounded">Current: {String(form[fieldKey])}</span>
            <span className="text-xs text-green-300 bg-green-500/10 px-2 py-0.5 rounded">New: {String(subVal)}</span>
            <button onClick={()=>set(fieldKey, subVal)} className="text-xs bg-green-600 hover:bg-green-500 text-white px-2 py-0.5 rounded transition">Use New</button>
        ...
        {newVal && (
          <div className="flex gap-2 mb-1.5">
            <button onClick={()=>set(fieldKey, subVal)} className="text-xs bg-green-600 hover:bg-green-500 text-white px-2 py-0.5 rounded transition">Accept: {String(subVal)}</button>
          </div>
        )}
        {options ? (
          ...
          <input type={type} value={form[fieldKey]||''} onChange={e=>set(fieldKey,e.target.value)} className="w-full bg-gray-800 border border-gray-700 text-white rounded px-3 py-1.5 text-xs focus:outline-none focus:border-blue-500" />
        )}
      </div>
    );
  };
        ...
        <div className="px-6 py-4 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Buying Status" fieldKey="buyingStatus" options={[{v:'',l:'Unknown'},{v:'ACTIVELY_BUYING',l:'🔥 Actively Buying'},{v:'BUYING_SELECTIVELY',l:'👀 Selectively'},{v:'PAUSED',l:'⏸ Paused'},{v:'JUST_LOOKING',l:'👋 Just Looking'}]} />
              <Field label="Monthly Capacity" fieldKey="monthlyCapacity" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Primary Market" fieldKey="marketPrimary" />
              <Field label="Secondary Markets" fieldKey="marketSecondary" />
              <Field label="States" fieldKey="states" />
              <Field label="Zip Codes" fieldKey="zipCodes" />
              <div className="col-span-2">
                <Field label="Excluded Areas" fieldKey="excludedAreas" />
              </div>
            ...
            <div className="mb-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {['SFH','Duplex','Triplex','Fourplex','Multi-Family (5+)','Mobile Home','Condo','Townhouse','Commercial'].map(pt => {
                  ...
                  return <button key={pt} onClick={()=>{ const cur = form.propertyTypes ? form.propertyTypes.split(',').map((s:string)=>s.trim()).filter(Boolean) : []; const updated = selected ? cur.filter((s:string)=>s!==pt) : [...cur,pt]; set('propertyTypes', updated.join(', ')); }} className={`text-xs px-2.5 py-1 rounded-full border transition ${selected?'bg-blue-600 text-white border-blue-500':'bg-gray-800 text-gray-400 border-gray-700'}`}>{pt}</button>;
                })}
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <Field label="Min Beds" fieldKey="minBeds" type="number" />
              <Field label="Min Year Built" fieldKey="minYearBuilt" type="number" />
              <Field label="HOA OK?" fieldKey="hoaOk" options={[{v:'',l:'Unknown'},{v:'Yes',l:'Yes'},{v:'No',l:'No'}]} />
              <div className="col-span-3">
                <Field label="Occupancy" fieldKey="occupancy" options={[{v:'',l:'No preference'},{v:'Vacant only',l:'Vacant only'},{v:'Tenant-occupied OK',l:'Tenant-occupied OK'},{v:'No preference',l:'No preference'}]} />
              </div>
            </div>
          </div>
          ...
          {/* Price */}
          <div>
            <h3 className="text-gray-300 text-xs font-semibold uppercase tracking-wider mb-3">Price & Financials</h3>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Min Price" fieldKey="minPrice" type="number" />
              <Field label="Max Price" fieldKey="maxPrice" type="number" />
              <Field label="Min ARV" fieldKey="minArv" type="number" />
              <Field label="Min Profit" fieldKey="minProfit" type="number" />
              <Field label="Max EMD" fieldKey="maxEmd" type="number" />
              <Field label="Inspection Days" fieldKey="inspectionDays" type="number" />
              <div className="col-span-2 flex items-center gap-2">
          ...
          {/* Strategy */}
          <div>
            <h3 className="text-gray-300 text-xs font-semibold uppercase tracking-wider mb-3">Strategy & Rehab</h3>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Strategies (comma separated)" fieldKey="strategies" />
              <Field label="Rehab Tolerance" fieldKey="rehabTolerance" options={[{v:'',l:'Unknown'},{v:'COSMETIC_ONLY',l:'Turnkey only'},{v:'LIGHT',l:'Light rehab'},{v:'MEDIUM',l:'Medium rehab'},{v:'HEAVY',l:'Heavy rehab'},{v:'FULL_GUT',l:'Full gut'}]} />
              <div className="col-span-2">
                <Field label="Hard No Criteria" fieldKey="hardNoCriteria" />
              </div>
            </div>
          </div>
          ...
          {/* Funding */}
          <div>
            <h3 className="text-gray-300 text-xs font-semibold uppercase tracking-wider mb-3">Funding & Closing</h3>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Funding Types" fieldKey="fundingTypes" />
              <Field label="Close Speed (days)" fieldKey="closeSpeed" type="number" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Preferred Contact" fieldKey="preferredContact" options={[{v:'',l:'Unknown'},{v:'Text only',l:'Text only'},{v:'Text first, then call',l:'Text first, then call'},{v:'Call me',l:'Call me'},{v:'Email',l:'Email'}]} />
              <Field label="Deal Send Frequency" fieldKey="dealSendFreq" options={[{v:'',l:'Unknown'},{v:'Every deal',l:'Every deal'},{v:'Only strong matches',l:'Only strong matches'},{v:'Weekly digest',l:'Weekly digest'},{v:'Only top deals',l:'Only top deals'}]} />
            </div>
          </div>
          ...
          {d.freeformNotes && (
            <div>
              <h3 className="text-gray-300 text-xs font-semibold uppercase tracking-wider mb-3">Buyer Notes (from submission)</h3>
                ...
                <div key={sub.id}>
                  <div className="flex items-center justify-between px-4 py-3 hover:bg-gray-800/40 transition">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={()=>{ setSelectedSub(sub); setFieldDecisions({}); }} className="px-3 py-1.5 text-xs rounded-lg transition font-medium bg-purple-700 hover:bg-purple-600 text-white">
                        ▼ Review
                      </button>
                    </div>
                  </div>
