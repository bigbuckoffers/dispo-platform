...
export default function BuyersPage() {
  ...
  const name = (b: any) => (!b.firstName||b.firstName==='Unknown') ? (b.phone||b.email?.split('@')[0]||'Unknown') : b.lastName==='Buyer' ? b.firstName : `${b.firstName} ${b.lastName}`.trim();

  const needsReview = [...buyers].filter(b => profileScore(b) < 70).sort((a,b) => (b.compositeScore||0)-(a.compositeScore||0));
    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      ...
      <div className="flex gap-1 mb-6 bg-gray-900 border border-gray-800 rounded-xl p-1 w-fit">
        ...
        <button onClick={() => setTab('review')} className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${tab==='review'?'bg-orange-600 text-white':'text-gray-400 hover:text-white'}`}>
          Needs Review <span className="ml-1 text-xs bg-orange-500/30 text-orange-300 px-1.5 py-0.5 rounded-full">{needsReview.length}</span>
        </button>
      </div>
            ...
            <tbody>
              ...
              : buyers.map((b:any)=>(
                <tr key={b.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 cursor-pointer" onClick={()=>window.location.href=`/dashboard/buyers/${b.id}`}>
                  <td className="px-4 py-3"><div className="font-medium">{name(b)}</div><div className="text-gray-400 text-xs">{b.email?.includes('@import.dispoai.com')?(b.phone||''):b.email}</div></td>
      ...
      {tab==='review' && (
        <div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-gray-800">{['Buyer','Score','Profile Complete','Missing Info','Market'].map(h=><th key={h} className="text-left text-gray-400 font-medium px-4 py-3">{h}</th>)}</tr></thead>
              <tbody>
                {loading ? [...Array(5)].map((_,i)=><tr key={i} className="border-b border-gray-800/50">{[...Array(5)].map((_,j)=><td key={j} className="px-4 py-3"><div className="h-4 bg-gray-800 rounded animate-pulse"/></td>)}</tr>)
                ...
                : needsReview.map((b:any)=>{
                  ...
                  const barColor=ps>=70?'bg-green-500':ps>=40?'bg-yellow-500':'bg-red-500';
                  return (
                    <tr key={b.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 cursor-pointer" onClick={()=>window.location.href=`/dashboard/buyers/${b.id}`}>
                      <td className="px-4 py-3"><div className="font-medium">{name(b)}</div><div className="text-gray-400 text-xs">{b.phone||''}</div></td>
                      <td className="px-4 py-3"><span className={`font-bold ${sc(b.compositeScore)}`}>{b.compositeScore}</span></td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-20 bg-gray-700 rounded-full h-1.5"><div className={`${barColor} h-1.5 rounded-full`} style={{width:`${ps}%`}}/></div>
