          ...
          <div className="col-span-12 md:col-span-3" style={{height:420}}>
            <div className="bg-gray-900 rounded-xl border border-gray-800 h-full flex flex-col overflow-hidden">
              <div className="px-3 py-2.5 border-b border-gray-800 flex items-center justify-between">
                <span className="text-white text-xs font-semibold">Deal Quality</span>
                  ...
                  return (
                    <>
                      ...
                      <div className="flex items-center gap-3 mb-3 pb-3 border-b border-gray-800">
                        <div className="relative shrink-0">
                          <svg width="64" height="64" viewBox="0 0 64 64">
                            ...
                            <circle cx="32" cy="32" r="28" fill="none" stroke={dqColor} strokeWidth="6"
                              strokeDasharray={circ} strokeDashoffset={offset}
                              strokeLinecap="round" transform="rotate(-90 32 32)" style={{transition:'stroke-dashoffset 0.6s ease'}}/>
                          </svg>
                          <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-white font-bold text-sm">{dqs}</span>
                          </div>
                        </div>
                        <div>
                          <p className={`font-bold text-sm ${dqTextColor}`}>{dqLabel}</p>
                          <p className="text-gray-500 text-[10px]">Deal Quality Score</p>
                        </div>
                      </div>

                      {/* Pricing vs Public Value */}
                      ...
                      <div className="space-y-2">
                        ...
                        {mao !== null && (
                          <div className={`rounded-lg px-2 py-1.5 text-center mt-1 ${askVsArv!==null&&askVsArv<=0?'bg-green-900/30':'bg-yellow-900/20'}`}>
                            ...
                            {askVsArv !== null && (
                              <p className={`text-[10px] font-semibold mt-0.5 ${askVsArv<=0?'text-green-400':'text-red-400'}`}>
                                {askVsArv<=0?`Ask is ${formatCurrency(Math.abs(askVsArv))} under MAO ✓`:`Ask is ${formatCurrency(askVsArv)} over MAO`}
                              </p>
                            )}
                          </div>
                        )}
                        {!deal.arv && (
                          <p className="text-gray-700 text-[10px] text-center mt-2">Enter ARV to calculate MAO</p>
                        )}
                      </div>
                    </>
                  );
